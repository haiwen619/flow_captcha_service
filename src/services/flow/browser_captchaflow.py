...
def _coerce_browser_proxy_url_to_uri(proxy_url: str) -> Optional[str]:
    """Coerce common proxy notations into a URI string."""
    if not proxy_url:
        return None
    ...
    scheme_match = re.match(r'^(socks5|http|https)://', raw_proxy)
    scheme = scheme_match.group(1) if scheme_match else "http"
    remainder = raw_proxy[len(scheme_match.group(0)):] if scheme_match else raw_proxy
...
def normalize_browser_proxy_url(proxy_url: str) -> tuple[Optional[str], Optional[str]]:
    ...
    proxy_url = _coerce_browser_proxy_url_to_uri(proxy_url)
    if not proxy_url:
        return None, None
    ...
    if protocol not in {"http", "https", "socks5"} or not host or port is None:
